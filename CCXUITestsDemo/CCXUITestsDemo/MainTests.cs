...
namespace CCXUITestsDemo
{
    ...
    public class MainTests
    {
        ...
        public string globalTenderTitle = "";
        public string globalTenderRefNum = "";
        ...
        public static IWebElement btnRefresh_ManAllocScenario;
        public static IWebElement btnAnalysis_ManAllocScenario;
        public static IWebElement btnVisibleFields_ManAllocScenario;
        public static IWebElement btnSelectAllocMode_ManAllocScenario;
        public static IWebElement btnViewAllocMode_ManAllocScenario;
        public static IWebElement btnSaveAllocs_ManAllocScenario;
        public static IWebElement btnCompleteOffer_ManAllocScenario;
        public static String workPathShipperMatrix;
        public static String workPathCarrierMatrix1;
        public static String workPathCarrierMatrix2;
        public static String workPathCarrierMatrix3;

        public static IWebElement dlgTenderComplete;
        public static IWebElement formFinishPublishingTender;
        public static IWebElement wizardTenderCreation;
      ...
      //  [TestMethod]
        ...
        [TestMethod]
        public void Test_SmokeTesting()
        {
            logger.Info("Start Test_SmokeTesting test");
            switch (browser)
            {
                case "ie":
        ...
        [TestMethod]
        public void Test_NextRoundTendering()
        {
            logger.Info("Start Next_Round_Tendering test");
            switch (browser)
            {
                case "ie":
                    RunIENextRoundTenderingTest();
                    break;
                case "ff":
                    RunFireFoxCreateTenderTestt();
                    break;
     ...
     //   [TestMethod]
      ...
      //  [TestMethod]
        ...
        private void CreateTenderProcessStep5()
        {
            ...
            IJavaScriptExecutor executor = (IJavaScriptExecutor)driver;
            /*
            // local path to CCX file  
            string projectCurrentPath2 = Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().CodeBase);
            var FilePath4 = Path.Combine(projectCurrentPath2, "binary_files\\Default.ccx");
            workPathShipperMatrix = new Uri(FilePath4).LocalPath;
            Console.WriteLine("Actual Shipper's Matrix Path {0}", workPathShipperMatrix);*/
            ...
            //executor.ExecuteScript("objShippingMatrix.Content.ShippingMatrixClient.ImportTestMatrix('"+ workPathShipperMatrix + "');");
            Thread.Sleep(7000);
        ...
        private void CreateTenderProcessStep9()
        {
            ...
            wizardTenderCreation = driver.FindElement(By.XPath("//div[@id='tenderWizardDlg']"));
            ...
            //Wait till the FINISH - PUBLISHING TENDER form is loaded
            explicitWaitsUntilElementLocated(15, "//span[contains(text(), 'Publish tender')]", "CREATE NEW TENDER wizard - STEP# 9 - FINISH PUBLISHING TENDER Form");
            Thread.Sleep(1000);


            //Take screenshot of "Create New Scenario - Step# 9 - FINISH PUBLISHING TENDER Form"
            TakeScreenshotOfWebElement("//div[contains(@style,'width: 600px; height: 225px')]", "actual_shipper_CreateNewTender_Step9_Finish_Publishing_Tender_Form.png");
        
            //Compare master and actual screenshots and create a separate screenshot with differences
            CompareScreenshots2("master_shipper_CreateNewTender_Step9_Finish_Publishing_Tender_Form.png", "actual_shipper_CreateNewTender_Step9_Finish_Publishing_Tender_Form.png", "difference_shipper_CreateNewTender_Step9_Finish_Publishing_Tender_Form.png", 10);
            
            System.Threading.Thread.Sleep(1000);



            //Click "Hereby I agree..." check-box 
            ...
            //Click on "Publish tender" button on the FINISH PUBLISHING TENDER Form
            var saveBtn = driver.FindElement(By.XPath("//*[@id='SaveBtn-btnEl']"));
            executor.ExecuteScript("arguments[0].click();", saveBtn);

            //Wait till the TENDER COMPLETE dialog is loaded
            int dlgShown = 0;
            var waitS9 = new WebDriverWait(driver, TimeSpan.FromSeconds(15)); // seconds
            var startTimeS9 = System.Diagnostics.Stopwatch.StartNew();

            
            try
            {

                waitS9.Until(ExpectedConditions.ElementIsVisible(By.XPath("//div[contains(text(), 'Your tender now will be reviewed by administrator. This process will take up to 24 hours.')]")));

                startTimeS9.Stop();
                var elapsedTimeInSecondsS9 = startTimeS9.Elapsed.Milliseconds;
                var resS9 = (double)elapsedTimeInSecondsS9 / 1000;
                formFinishPublishingTender = driver.FindElement(By.XPath("//div[contains(text(), 'Your tender now will be reviewed by administrator. This process will take up to 24 hours.')]"));
                dlgShown = 1;


                logger.Info("TOTAL TIME that CREATE NEW TENDER wizard - STEP# 9 - TENDER COMPLETE Dialog was loading: " + resS9.ToString() + " seconds.");
                //return;

            }
            catch (Exception e)
            {
                logger.Error("CREATE NEW TENDER wizard - STEP# 9 - TENDER COMPLETE Dialog has timed out after " + 15 + " seconds: " + e.Message);
                driver.Quit();
            ...
            if (dlgShown > 0)
            {
                ...
                dlgTenderComplete = driver.FindElement(By.XPath("//div[@id = 'messagebox-1001']"));

                //Take screenshot of "Create New Tender - Step# 9 - TENDER COMPLETE Dialog"
                TakeScreenshotOfWebElement("//div[@id = 'messagebox-1001']", "actual_shipper_CreateNewTender_Step9_Tender_Complete_Dialog.png");

                //Compare master and actual screenshots and create a separate screenshot with differences
                CompareScreenshots2("master_shipper_CreateNewTender_Step9_Tender_Complete_Dialog.png", "actual_shipper_CreateNewTender_Step9_Tender_Complete_Dialog.png", "difference_shipper_CreateNewTender_Step9_Tender_Complete_Dialog.png", 10);

                System.Threading.Thread.Sleep(1000);

                
                // var ok = driver.FindElement(By.XPath("//*[contains(text(), 'OK')]"));
                var ok = driver.FindElement(By.XPath("//span[contains(text(), 'OK')]"));

                //Click "OK" button on TENDER COMPLETE dialog        
                executor.ExecuteScript("arguments[0].click();", ok);

                System.Threading.Thread.Sleep(2000);

                if (!dlgTenderComplete.Displayed)
                {
                    if (!formFinishPublishingTender.Displayed)
                    {
                        if (driver.FindElements(By.XPath("//div[@id='tenderWizardDlg']")).Count() == 0) // check if web-element is still displayed/open
                        {                            
                            return;
                        }
                        else
                        {
                            logger.Warn("CREATE NEW TENDER wizard - STEP# 9 - TENDER CREATION Wizard is not closed.");
                            driver.Quit();
                        }
                    }
                    else
                    {
                        logger.Warn("CREATE NEW TENDER wizard - STEP# 9 - FINISH PUBLISHING TENDER Form is not closed.");
                        driver.Quit();
                    }
                }
                else {
                    logger.Error("CREATE NEW TENDER wizard - STEP# 9 - TENDER COMPLETE Dialog is not closed.");
                    driver.Quit();
                }
            }
        }
        ...
        private void FindAndOpenTenderWithAnalysis()
        {
            ...
            //txtFilterTenderRefNum.SendKeys("T1805060629");
            //txtFilterTenderRefNum.SendKeys("T1805071113");
            //txtFilterTenderRefNum.SendKeys("T1807161150");
        ...
        private void FindAndOpenTenderWithAnalysisForNextTendering()
        {
            ...
            //Click on "X" to close "Tender" tab
            var tabTender = driver.FindElement(By.XPath("//div[@id='tpMain']/div[1]/div[1]/div[2]/div/div[2]/a[@class='x-tab-close-btn']"));
            executor.ExecuteScript("arguments[0].click();", tabTender);
            ...
            //Enter tender's Reference number into search field
            //globalTenderRefNum = "T1807230951";
            var txtFilterTenderRefNum = driver.FindElement(By.XPath("//td[@id='filterRef-bodyEl']/input[@id='filterRef-inputEl']"));
            txtFilterTenderRefNum.Clear();
            txtFilterTenderRefNum.SendKeys(globalTenderRefNum);  //Tender Reference Number
            //txtFilterTenderRefNum.SendKeys("T1807230951"); //next tendering tender


            //Clear tender's title search field
            var txtFilterTenderName = driver.FindElement(By.XPath("//*[@id='filtertdrInfo-inputEl']"));
            txtFilterTenderName.Clear();  
            ...
            if (tenderStatus.Equals("Ready for analysis"))
            {
                ...
                //Click on tender's "Extra menu" button
                var btnViewTenderExtraMenu = driver.FindElement(By.XPath("//div[@id='ShipperGridPanel-body']/div/table/tbody/tr[2]/td/div/table/tbody/tr/td[14]/div/div/div[@class='row-imagecommand   icon-folder ']"));
                executor.ExecuteScript("arguments[0].click();", btnViewTenderExtraMenu);
                //action.MoveToElement(btnViewTenderExtraMenu).Click().Build().Perform();
                Thread.Sleep(1000);
                ...
                //Wait till the "Tender's EXTRA MENU" popup is loading
                explicitWaitsUntilElementLocated(15, "//span[contains(text(), 'Next Round')]", "TENDER's EXTRA MENU popup");


                //Click on "Next Round" item in the Tender's Extra Menu popup
                //"//div[@class='x-panel x-layer x-panel-default x-menu']/div/div[2]/div[2]/div[@class='x-component x-box-item x-component-default x-menu-item']"
                var btnNextRound = driver.FindElement(By.XPath("//div[@class='x-component x-box-item x-component-default x-menu-item']/a/span[contains(text(), 'Next Round')]"));
                action.MoveToElement(btnNextRound).Click().Build().Perform();


                driver.SwitchTo().DefaultContent();

                //Wait till the "Preparation of the next tendering round" form is loading
                explicitWaitsUntilElementLocated(15, "//div[@id='tnrScenarios-body']/div/table/tbody/tr[4]/td/div[contains(text(), 'ManAlloc_SOC')]", "PREPARATION of the NEXT TENDERING ROUND form");

                //Thread.Sleep(1000);

                //Take screenshot of "Preparation of the next tendering round" form
                TakeScreenshotOfWebElement("//div[contains(@style,'width: 750px; height: 520px;')]", "actual_shipper_Preparation_Form_NextTendering.png");
                
                //Compare master and actual screenshots and create a separate screenshot with differences
                CompareScreenshots2("master_shipper_Preparation_Form_NextTendering.png", "actual_shipper_Preparation_Form_NextTendering.png", "difference_shipper_Preparation_Form_NextTendering.png", 10);
        ...
        private void CompletePreparationOfNextTenderingRound() {
            ...
            ///////////////////////////////////////////////////////////////////////////////
            //  "Preparation of the next tendering round"- Step 1/2 form
            ///////////////////////////////////////////////////////////////////////////////
            
            //Click on "ManAlloc_SOC" scenario in the left-side panel of the "Preparation of the next tendering round"- Step 1/2 form
            executor.ExecuteScript("arguments[0].click();", driver.FindElement(By.XPath("//div[@id='tnrScenarios-body']/div/table/tbody/tr[4]/td/div[contains(text(), 'ManAlloc_SOC')]")));


            //Wait till the tender structure is loaded in the right panel of the "Preparation of the next tendering round" form - Step 1/2 
            explicitWaitsUntilElementLocated(15, "//div[contains(text(), '30%')]", "PREPARATION of the NEXT TENDERING ROUND form - Right-side Panel - Step#1");

            Thread.Sleep(1000);
                        

            //Click on the top check-box to select the whole tender's structure levels 
            action.MoveToElement(driver.FindElement(By.XPath("//div[@id='tnrAwardingAllocations-body']/div/table/tbody/tr[2]/td[1]/div/div"))).Click().Build().Perform();
            Thread.Sleep(1000);

            int countCheck = 0;

            //Check whether the "Next" button is enabled and click on it
            if (driver.FindElement(By.XPath("//button[@id='nextBtn-btnEl']")).Enabled)
            {

                //Take screenshot of "Preparation of the next tendering round" form
                TakeScreenshotOfWebElement("//div[contains(@style,'width: 750px; height: 520px;')]", "actual_shipper_Preparation_Form_NextTendering_Step#1_Completed.png");

                //Compare master and actual screenshots and create a separate screenshot with differences
                CompareScreenshots2("master_shipper_Preparation_Form_NextTendering_Step#1_Completed.png", "actual_shipper_Preparation_Form_NextTendering_Step#1_Completed.png", "difference_shipper_Preparation_Form_NextTendering_Step#1_Completed.png", 10);

                
                //Click on "Next" button
                //action.MoveToElement(driver.FindElement(By.XPath("//button[@id='nextBtn-btnEl']"))).Click().Build().Perform();
                executor.ExecuteScript("arguments[0].click();", driver.FindElement(By.XPath("//button[@id='nextBtn-btnEl']")));

                //Wait till the tender structure is loaded on the "Preparation of the next tendering round" form - Step 2/2 
                explicitWaitsUntilElementLocated(15, "//div[@id='tnrTenderStructure-body']/div/table/tbody/tr[10]/td/div[contains(text(), 'Custom requirements')]", "PREPARATION of the NEXT TENDERING ROUND form - Step#2");
                ...
                logger.Error("NEXT button is still disable on the 'Preparation of the next tendering round' form - Step#1.");
                driver.Quit();
            ...
            if (countCheck > 0)
            {
                ...
                //Take screenshot of "Preparation of the next tendering round" form
                TakeScreenshotOfWebElement("//div[contains(@style,'width: 750px; height: 520px;')]", "actual_shipper_Preparation_Form_NextTendering_Step#2_Completed.png");

                //Compare master and actual screenshots and create a separate screenshot with differences
                CompareScreenshots2("master_shipper_Preparation_Form_NextTendering_Step#2_Completed.png", "actual_shipper_Preparation_Form_NextTendering_Step#2_Completed.png", "difference_shipper_Preparation_Form_NextTendering_Step#2_Completed.png", 10);


                //Click on "Create" button
                //action.MoveToElement(driver.FindElement(By.XPath("//button[@id='finishBtn-btnEl']"))).Click().Build().Perform();
                executor.ExecuteScript("arguments[0].click();", driver.FindElement(By.XPath("//button[@id='finishBtn-btnEl']")));
                ...
                logger.Error("Unable to open 'Preparation of the next tendering round' form - Step#2.");
                driver.Quit();
            }
        }
        ...
        private void FindCreatedNextRoundTender() {

            // use js executor as click have problem in IE
            IJavaScriptExecutor executor = (IJavaScriptExecutor)driver;

            //Check if the "Preparation of the next tendering round" form - Step 2/2 was closed 
            if (driver.FindElements(By.XPath("//div[@id='tenderWizardDlg']")).Count() == 0) // check if web-element is still displayed/open
            {
                ...
                //Clean the Tender REF field
                driver.FindElement(By.XPath("//input[@id='filterRef-inputEl']")).Clear();
                ...
                Thread.Sleep(1000);
                txtFilterTenderName.SendKeys("Next round: " + globalTenderTitle);  
                ...
                //If tender's status is "Preparation" then proceed
                if (!tenderStatus.Equals("Preparation"))
                {
                    logger.Warn("After creating Next Round Tender by the shipper the tender's status should be 'PREPARATION'. However, current tender's status is " + tenderStatus.ToUpper());
                    driver.Quit();
                }
            ...
            {
                logger.Error("'PREPARATION OF THE NEXT TENDERING ROUND' form is not closed.");
                driver.Quit();
            }
        ...
        private void EditAndPublishNextRoundTender() {
            ClickOnEditTenderButton();
            WaitForAjax("Create tender");
            EditEndOfTenderDateTime_Step1();
            Checking_Step2();
            Checking_Step3();
            Checking_Step4();
            Checking_Step5();
        ...
        private void ClickOnEditTenderButton()
        {
            // use js executor as click have problem in IE
            IJavaScriptExecutor executor = (IJavaScriptExecutor)driver;
            ...
            //Click on tender's "Edit" button
            var btnTenderEdit = driver.FindElement(By.XPath("//div[@id='ShipperGridPanel-body']/div/table/tbody/tr[2]/td/div/table/tbody/tr/td[14]/div/div/div[2]"));
            executor.ExecuteScript("arguments[0].click();", btnTenderEdit);
                        
            Thread.Sleep(1000);

            driver.SwitchTo().DefaultContent();

            //Wait till the step#1 of the tender's wizard is loaded 
            explicitWaitsUntilElementLocated(15, "//div[@id='tenderWizardDlg-body']", "Editing NEXT ROUND TENDER wizard - Step#1");

            Thread.Sleep(1000);

            //Take screenshot of NEXT ROUND TENDER wizard - Step#1 form
            TakeScreenshotOfWebElement("//div[contains(@style,'width: 980px; height: 500px;')]", "actual_shipper_NextTendering_Editing_Step#1.png");

            //Compare master and actual screenshots and create a separate screenshot with differences
            CompareScreenshots2("master_shipper_NextTendering_Editing_Step#1.png", "actual_shipper_NextTendering_Editing_Step#1.png", "difference_shipper_NextTendering_Editing_Step#1.png", 10);
            
        }



        private void EditEndOfTenderDateTime_Step1() {
            ...
            var next = driver.FindElement(By.XPath("//div[@id='tenderWizardDlg']/div[3]/div/div/div[4]/em/button/span[contains(text(), 'Next')]"));
            executor.ExecuteScript("arguments[0].click();", next);
            //action.MoveToElement(driver.FindElement(By.XPath("//button[@id='nextBtn-btnEl']"))).Click().Build().Perform();
        ...
        private void Checking_Step2() {

            IJavaScriptExecutor executor = (IJavaScriptExecutor)driver;

            //Wait till the step#2 of the tender's wizard is loaded 
            explicitWaitsUntilElementLocated(15, "//label[@id='ShowCompanyName-boxLabelEl']", "Editing NEXT ROUND TENDER wizard - Step#2");

            Thread.Sleep(1000);

            //Take screenshot of NEXT ROUND TENDER wizard - Step#2 form
            TakeScreenshotOfWebElement("//div[@id='tenderWizardDlg']", "actual_shipper_NextTendering_Editing_Step#2.png");

            //Compare master and actual screenshots and create a separate screenshot with differences
            CompareScreenshots2("master_shipper_NextTendering_Editing_Step#2.png", "actual_shipper_NextTendering_Editing_Step#2.png", "difference_shipper_NextTendering_Editing_Step#2.png", 10);
        ...
        private void Checking_Step3()
        {

            IJavaScriptExecutor executor = (IJavaScriptExecutor)driver;

            //Wait till the step#3 of the tender's wizard is loaded 
            explicitWaitsUntilElementLocated(15, "//div[@id='AttachmentsToolbarStp3-innerCt']", "Editing NEXT ROUND TENDER wizard - Step#3");

            Thread.Sleep(1000);

            //Take screenshot of NEXT ROUND TENDER wizard - Step#3 form
            TakeScreenshotOfWebElement("//div[@id='tenderWizardDlg']", "actual_shipper_NextTendering_Editing_Step#3.png");

            //Compare master and actual screenshots and create a separate screenshot with differences
            CompareScreenshots2("master_shipper_NextTendering_Editing_Step#3.png", "actual_shipper_NextTendering_Editing_Step#3.png", "difference_shipper_NextTendering_Editing_Step#3.png", 10);
        ...
        private void Checking_Step4()
        {

            IJavaScriptExecutor executor = (IJavaScriptExecutor)driver;

            //Wait till the step#4 of the tender's wizard is loaded 
            explicitWaitsUntilElementLocated(15, "//table[@id='TruckEquipmentCombo-triggerWrap']", "Editing NEXT ROUND TENDER wizard - Step#4");

            Thread.Sleep(1000);

            //Take screenshot of NEXT ROUND TENDER wizard - Step#4 form
            TakeScreenshotOfWebElement("//div[@id='tenderWizardDlg']", "actual_shipper_NextTendering_Editing_Step#4.png");

            //Compare master and actual screenshots and create a separate screenshot with differences
            CompareScreenshots2("master_shipper_NextTendering_Editing_Step#4.png", "actual_shipper_NextTendering_Editing_Step#4.png", "difference_shipper_NextTendering_Editing_Step#4.png", 10);
        ...
        private void Checking_Step5()
        {

            IJavaScriptExecutor executor = (IJavaScriptExecutor)driver;

            //Wait till the step#5 of the tender's wizard is loaded 
            explicitWaitsUntilElementLocated(15, "//div[@id='silverlightControlHost']", "Editing NEXT ROUND TENDER wizard - Step#5");

            Thread.Sleep(1000);

            //Click on "X" button to bring the wizard to full screen
            var headerDiv = driver.FindElement(By.Id("tenderWizardDlg_header-innerCt"));
            var fullScreen = headerDiv.FindElements(By.TagName("div"))[0].FindElements(By.TagName("img"))[0];
            executor.ExecuteScript("arguments[0].click();", fullScreen);


            //Take screenshot of NEXT ROUND TENDER wizard - Step#5 form
            TakeScreenshotOfWebElement("//div[@id='tenderWizardDlg']", "actual_shipper_NextTendering_Editing_Step#5.png");
            Thread.Sleep(1000);

            //Wait till the bidding matrix is loaded
            var watch45 = System.Diagnostics.Stopwatch.StartNew();

            while (CompareScreenshotsMatrix("master_shipper_NextTendering_Editing_Step#5.png", "actual_shipper_NextTendering_Editing_Step#5.png", 40) > 25)
            {
                //wait next 3 seconds
                Thread.Sleep(1500);

                //Delete existing "actual_shipper_NextTendering_Editing_Step#5.png" screenshot
                 DeleteScreenshot("actual_shipper_NextTendering_Editing_Step#5.png");

                //Create new "actual_shipper_NextTendering_Editing_Step#5.png" screenshot
                TakeScreenshotOfWebElement("//div[@id='tenderWizardDlg']", "actual_shipper_NextTendering_Editing_Step#5.png");
            }

            watch45.Stop();
            var elapsedMs45 = watch45.ElapsedMilliseconds;
            var res45 = (double)elapsedMs45 / 1000;

            logger.Info("BIDDING MATRIX for NEXT ROUND TENDER was loading during " + res45 + " seconds.");

            if (res45 > 25)
            {
                logger.Warn(string.Format("It took quite a long time for the BIDDING MATRIX to load for NEXT ROUND TENDER: {0} sec!", res45.ToString()));
            }
            else {
                var next = driver.FindElement(By.XPath("//div[@id='tenderWizardDlg']/div[3]/div/div/div[4]/em/button/span[contains(text(), 'Next')]"));
                executor.ExecuteScript("arguments[0].click();", next);
        ...
        private void Checking_Step6()
        {
            Actions action = new Actions(driver);
            IJavaScriptExecutor executor = (IJavaScriptExecutor)driver;

            //Wait till the step#6 of the tender's wizard is loaded 
            explicitWaitsUntilElementLocated(15, "//div[@id='MyServiceProvidersGroupsGrid']", "Editing NEXT ROUND TENDER wizard - Step#6");

            Thread.Sleep(1000);


            //Click on "All Providers" group
            //executor.ExecuteScript("arguments[0].click();", driver.FindElement(By.XPath("//div[contains(text(), 'All Providers')]")));
            action.MoveToElement(driver.FindElement(By.XPath("//div[contains(text(), 'All Providers')]"))).Click().Build().Perform();

            //Wait till the lower "Providers" grid is loaded 
            explicitWaitsUntilElementLocated(15, "//div[@id='SubmitFormPanel-body']/div/div/div/div/div/div[3]/div[2]/div/table/tbody/tr[2]/td[1]/div/div", "Editing NEXT ROUND TENDER wizard - Step#6 - lower 'Providers' grid");

            Thread.Sleep(1000);
                                    


            //Check whether 2 carriers from the previous tender are selected in the lower "Providers" grid
            IList<IWebElement> available_Carriers = driver.FindElements(By.XPath("//div[@id='SubmitFormPanel-body']/div/div/div/div/div/div[3]/div[2]/div/table/tbody/descendant::tr"));

           
            int selectedCarriers = 0;

            int RowIndex = 1;


            //Loop all the carriers in the the lower "Providers" grid
            foreach (IWebElement item in available_Carriers)
            {
                String selectionStatus = item.GetAttribute("class").ToString();
               // Console.WriteLine("Carrier name: " + selectionStatus);

                if (selectionStatus.Contains("x-grid-row-selected"))
                {
                    String carrierName = item.FindElement(By.XPath("td[2]/div")).Text;
                    //Console.WriteLine("Carrier name: " + carrierName);

                    if (carrierName.Contains("CCX DEMO Carrier 04 (Lee Brant)") || carrierName.Contains("CCX DEMO Carrier 05 (Thomas Roberts)"))
                    {

                        selectedCarriers++;
                        logger.Info(carrierName+" is a participant of the NEXT ROUND TENDER.");

                    }
                    else {
                        logger.Error("LEE BRANT or THOMAS ROBERTS are NOT SELECTED as participants of the NEXT ROUND TENDER");
                        driver.Quit();
                    }
                ...
                else
                {
                    ...
                    foreach (IWebElement colElement in TotalColumnCount)
                    {
                        ...
                        //Retrieve names of displayed carriers
                        String carrierName = colElement.FindElement(By.XPath("div")).Text;
                       // Console.WriteLine("Carrier name: " + carrierName);


                        switch (carrierName)
                        {
                            case "CCX DEMO Carrier 07 (Christian Ogilvie)":

                                //Click to select Christian Ogilvie from the lower "Providers" grid
                                //executor.ExecuteScript("arguments[0].click();", driver.FindElement(By.XPath("//div[contains(text(), 'All Providers')]")));
                                var checkbox = driver.FindElement(By.XPath("//div[@id='SubmitFormPanel-body']/div/div/div/div/div/div[3]/div[2]/div/table/tbody/tr[" + RowIndex + "]/td[1]/div"));
                                action.MoveToElement(checkbox).Click().Build().Perform();

                                var rowStatus = driver.FindElement(By.XPath("//div[@id='SubmitFormPanel-body']/div/div/div/div/div/div[3]/div[2]/div/table/tbody/tr[" + RowIndex + "]"));
                                String selectionStatus2 = rowStatus.GetAttribute("class").ToString();
                                
                                if (selectionStatus2.Contains("x-grid-row-selected"))
                                {
                                    selectedCarriers++;
                                    logger.Info(carrierName + " is a participant of the NEXT ROUND TENDER.");
                                }
                                ...
                                {
                                    logger.Error("CHRISTIAN OGILVIE is NOT FOUND as in the lower 'Providers' grid.");
                                    driver.Quit();
                                }
                                break;
            ...
            //Take screenshot of NEXT ROUND TENDER wizard - Step#6 form
            TakeScreenshotOfWebElement("//div[@id='tenderWizardDlg']", "actual_shipper_NextTendering_Editing_Step#6.png");

            //Compare master and actual screenshots and create a separate screenshot with differences
            CompareScreenshots2("master_shipper_NextTendering_Editing_Step#6.png", "actual_shipper_NextTendering_Editing_Step#6.png", "difference_shipper_NextTendering_Editing_Step#6.png", 10);


            Console.WriteLine("selectedCarriers: " + selectedCarriers);
            if (selectedCarriers ==3) {
               var next = driver.FindElement(By.XPath("//div[@id='tenderWizardDlg']/div[3]/div/div/div[4]/em/button/span[contains(text(), 'Next')]"));
               executor.ExecuteScript("arguments[0].click();", next);
            }
        }
        ...
        private void Checking_Step7()
        {

            IJavaScriptExecutor executor = (IJavaScriptExecutor)driver;

            //Wait till the step#7 of the tender's wizard is loaded 
            explicitWaitsUntilElementLocated(15, "//div[@id='NotificatoinGrid-body']/div/table/tbody/tr[2]/td[5]/div/div/div/div/div[3]/em/button", "Editing NEXT ROUND TENDER wizard - Step#7");

            Thread.Sleep(1000);


            //Click on "X" button to bring the wizard to concise screen
            var conciseScreen = driver.FindElement(By.XPath("//div[@id='tenderWizardDlg_header-targetEl']/div/img[@class='x-tool-restore']"));
            executor.ExecuteScript("arguments[0].click();", conciseScreen);

            Thread.Sleep(1000);

            //Take screenshot of NEXT ROUND TENDER wizard - Step#7 form
            TakeScreenshotOfWebElement("//div[@id='tenderWizardDlg']", "actual_shipper_NextTendering_Editing_Step#7.png");

            //Compare master and actual screenshots and create a separate screenshot with differences
            CompareScreenshots2("master_shipper_NextTendering_Editing_Step#7.png", "actual_shipper_NextTendering_Editing_Step#7.png", "difference_shipper_NextTendering_Editing_Step#7.png", 10);
        ...
        private void Checking_Step8()
        {

            IJavaScriptExecutor executor = (IJavaScriptExecutor)driver;

            //Wait till the step#8 of the tender's wizard is loaded 
            explicitWaitsUntilElementLocated(15, "//div[@id='IsCustomRequirements-legendTitle']", "Editing NEXT ROUND TENDER wizard - Step#8");

            Thread.Sleep(1000);
                        

            //Take screenshot of NEXT ROUND TENDER wizard - Step#8 form
            TakeScreenshotOfWebElement("//div[@id='tenderWizardDlg']", "actual_shipper_NextTendering_Editing_Step#8.png");

            //Compare master and actual screenshots and create a separate screenshot with differences
            CompareScreenshots2("master_shipper_NextTendering_Editing_Step#8.png", "actual_shipper_NextTendering_Editing_Step#8.png", "difference_shipper_NextTendering_Editing_Step#8.png", 10);
        ...
        private void Checking_Step9()
        {

            IJavaScriptExecutor executor = (IJavaScriptExecutor)driver;

            //Wait till the step#9 of the tender's wizard is loaded 
            explicitWaitsUntilElementLocated(15, "//button[@id='Template-btnEl']", "Editing NEXT ROUND TENDER wizard - Step#9");

            Thread.Sleep(1000);


            //Take screenshot of NEXT ROUND TENDER wizard - Step#9 form
            TakeScreenshotOfWebElement("//div[@id='tenderWizardDlg']", "actual_shipper_NextTendering_Editing_Step#9.png");

            //Compare master and actual screenshots and create a separate screenshot with differences
            CompareScreenshots2("master_shipper_NextTendering_Editing_Step#9.png", "actual_shipper_NextTendering_Editing_Step#9.png", "difference_shipper_NextTendering_Editing_Step#9.png", 10);
            ...
            //Wait till the FINISH - PUBLISHING TENDER form is loaded
            explicitWaitsUntilElementLocated(15, "//span[contains(text(), 'Publish tender')]", "NEXT ROUND TENDER wizard - STEP# 9 - FINISH PUBLISHING TENDER Form");
            Thread.Sleep(1000);


            //Take screenshot of "Create New Tender - Step# 9 - FINISH PUBLISHING TENDER Form"
            TakeScreenshotOfWebElement("//div[contains(@style,'width: 600px; height: 225px')]", "actual_shipper_NextRoundTender_Step9_Finish_Publishing_Tender_Form.png");

            //Compare master and actual screenshots and create a separate screenshot with differences
            CompareScreenshots2("master_shipper_NextRoundTender_Step9_Finish_Publishing_Tender_Form.png", "actual_shipper_NextRoundTender_Step9_Finish_Publishing_Tender_Form.png", "difference_shipper_NextRoundTender_Step9_Finish_Publishing_Tender_Form.png", 10);
            ...
            //Wait till the TENDER COMPLETE dialog is loaded
            int dlgShown = 0;
            var waitS10 = new WebDriverWait(driver, TimeSpan.FromSeconds(15)); // seconds
            var startTimeS10 = System.Diagnostics.Stopwatch.StartNew();


            try
            {

                waitS10.Until(ExpectedConditions.ElementIsVisible(By.XPath("//div[contains(text(), 'Your tender now will be reviewed by administrator. This process will take up to 24 hours.')]")));

                startTimeS10.Stop();
                var elapsedTimeInSecondsS10 = startTimeS10.Elapsed.Milliseconds;
                var resS10 = (double)elapsedTimeInSecondsS10 / 1000;
                formFinishPublishingTender = driver.FindElement(By.XPath("//div[contains(text(), 'Your tender now will be reviewed by administrator. This process will take up to 24 hours.')]"));
                dlgShown = 1;


                logger.Info("TOTAL TIME that NEXT ROUND TENDER wizard - STEP# 9 - TENDER COMPLETE Dialog was loading: " + resS10.ToString() + " seconds.");
                //return;

            }
            catch (Exception e)
            {
                logger.Error("NEXT ROUND TENDER wizard - STEP# 9 - TENDER COMPLETE Dialog has timed out after " + 15 + " seconds: " + e.Message);
                driver.Quit();
            ...
            if (dlgShown > 0)
            {
                ...
                //Take screenshot of "Create New Scenario - Step# 9 - TENDER COMPLETE Dialog"
                TakeScreenshotOfWebElement("//div[@id = 'messagebox-1001']", "actual_shipper_NextRoundTender_Step9_Tender_Complete_Dialog.png");

                //Compare master and actual screenshots and create a separate screenshot with differences
                CompareScreenshots2("master_shipper_NextRoundTender_Step9_Tender_Complete_Dialog.png", "actual_shipper_NextRoundTender_Step9_Tender_Complete_Dialog.png", "difference_shipper_NextRoundTender_Step9_Tender_Complete_Dialog.png", 10);
                ...
                if (!dlgTenderComplete.Displayed)
                {
                    if (!formFinishPublishingTender.Displayed)
                    {
                        ...
                        {
                            logger.Warn("NEXT ROUND TENDER wizard - STEP# 9 - TENDER CREATION Wizard is not closed.");
                            driver.Quit();
                        }
                    }
                    else
                    {
                        logger.Warn("NEXT ROUND TENDER wizard - STEP# 9 - FINISH PUBLISHING TENDER Form is not closed.");
                        driver.Quit();
                    }
                }
                else
                {
                    logger.Error("NEXT ROUND TENDER wizard - STEP# 9 - TENDER COMPLETE Dialog is not closed.");
                    driver.Quit();
                }
            }
        ...
        private void CreateManualAllocationScenarioWithFilters()
        {
           ...
           // waitTillDescendentElementsAvailable(15, "//div[@id='ScenarioTreePanel-body']/div/table/tbody/tr", "class", "x-grid-row  x-grid-tree-node-leaf", "OVERVIEW SCENARIOS tab - STEP#2");
            ...
            if (driver.FindElement(By.XPath("//div[@id='ScenarioTreePanel']/div[1]/div/div/div[1]")).GetAttribute("class").Contains("x-btn x-box-item x-toolbar-item x-btn-default-toolbar-small x-icon-text-left x-btn-icon-text-left x-btn-default-toolbar-small-icon-text-left"))
                                                                                                                                 
            {
                ...
                if (scenarioRulesActual.SequenceEqual(scenarioRulesExpected))
                {
                    ...
                    //Delete "Number of Carriers" rule
                    DeleteNumberOfCarriersRule();

                    //Click on "Finish" button to create the scenario
                    executor.ExecuteScript("arguments[0].click();", driver.FindElement(By.XPath("//button[@id='nextBtn-btnEl']/span[contains(text(),'Finish')]")));
                    ...
                    //Wait till the 'ManAlloc_SOC' scenario gets displayed on the "Overview Scenarios" tab
                    explicitWaitsUntilElementLocated(15, "//div/span[contains(text(),'ManAlloc_SOC')]", "OVERVIEW SCENARIOS tab - ManAlloc_SOC scenario added");

                    Thread.Sleep(1000);
                                        

                    //Take screenshot of "Overview Scenarios" tab with the 'ManAlloc_SOC' scenario added in the table
                    TakeScreenshotOfWebElement("//div[@id='ScenarioTreePanel']", "actual_shipper_OverviewScenariosTab_ManAlloc_SOC_scenario.png");

                    //Compare master and actual screenshots and create a separate screenshot with differences
                    CompareScreenshots2("master_shipper_OverviewScenariosTab_ManAlloc_SOC_scenario.png", "actual_shipper_OverviewScenariosTab_ManAlloc_SOC_scenario.png", "difference_shipper_OverviewScenariosTab_ManAlloc_SOC_scenario.png", 10);
        ...
        private void CreateManualAllocationScenarioWithoutFilters()
        {
            Actions action = new Actions(driver);
            ...
            var overviewScenariosTab = driver.FindElement(By.XPath("//div[@id='TenderStructurePanel-body']/div/table/tbody/tr/td/div[contains(text(),'Overview Scenarios')]"));
            ...
            explicitWaitsUntilElementLocated(15, "//div[@id='ScenarioTreePanel-body']/div/table/tbody/tr/td/div[contains(text(),'Germany')]", "OVERVIEW SCENARIOS tab - UnitPrice -> No Comparison table");
        ...
        private void MakingManualAllocationsInLowerGrid(int numberOfCarriers)
        {
            Actions action = new Actions(driver);
            IJavaScriptExecutor executor = (IJavaScriptExecutor)driver;
            ...
            Boolean found_ManAlloc_SOC = false;
            int numColumns = 0;
            
            try
            {
                //Check whether the 'ManAlloc_SOC' scenario item was added into DEFAULT folder in - NAVIGATION panel
                IList<IWebElement> available_AnalysisParts = driver.FindElements(By.XPath("//div[@id='TenderStructurePanel-body']/div/table/tbody/tr"));
                ...
                foreach (IWebElement item in available_AnalysisParts)
                {
                    ...
                    foreach (IWebElement colElement in TotalColumnCount)
                    {
                        ...
                        if (analysisPartName.Equals("ManAlloc_SOC"))
                        {
                            found_ManAlloc_SOC = true;
            ...
            {
                logger.Error("Problem encountered while reviewing available ANALYSIS items in DEFAULT folder in 'Navigation' panel in an attempt to find 'ManAlloc_SOC' scenario: " + e.Message);
                driver.Close();
                driver.Quit();
            }


            //In case the 'ManAlloc_SOC' scenario item is found in the DEFAULT folder
            if (found_ManAlloc_SOC == true)
            {
                //Click on the 'ManAlloc_SOC' scenario item
                executor.ExecuteScript("arguments[0].click();", driver.FindElement(By.XPath("//div[@id='TenderStructurePanel-body']/div/table/tbody/tr/td/div[contains(text(),'ManAlloc_SOC')]")));

                Thread.Sleep(1000);

                //Wait till the 'ManAlloc_SOC' scenario tab is loaded
                // waitTillDescendentElementsAvailable(15, "//*[name()='svg']/*[name()='rect']", "width", "12", "ManAlloc_SOC SCENARIO tab");
                //Find the specific pie div & get the text.
                //IWebElement meButton = driver.FindElement(By.CssSelector("//*[local-name()='svg' and namespace-uri()='http://www.w3.org/2000/svg']//*[local-name()='tspan' and text()='46200']"));
                ...
                //Check which tabs are currently opened in the Analysis
                try
                { 

                //Get the list of currently opened tabs in the Analysis
                IList<IWebElement> opened_AnalysisTabs = driver.FindElements(By.XPath("//div[@id='TabPanel']/div[@class='x-tab-bar x-docked x-tab-bar-default x-top x-tab-bar-top x-tab-bar-default-top x-horizontal x-tab-bar-horizontal x-tab-bar-default-horizontal x-docked-top x-tab-bar-docked-top x-tab-bar-default-docked-top x-unselectable']/div[@class='x-tab-bar-body x-tab-bar-body-top x-tab-bar-body-default-top x-tab-bar-body-horizontal x-tab-bar-body-default-horizontal x-tab-bar-body-default x-tab-bar-body-default-top x-tab-bar-body-default-horizontal x-tab-bar-body-default-docked-top x-box-layout-ct']/div[@class='x-box-inner x-horizontal-box-overflow-body']/div/descendant::div"));

                
                //Loop the tender's opened analysis tabs
                foreach (IWebElement item in opened_AnalysisTabs)
                {                   
                        //Retrieve names of available tender's analysis parts
                        String analysisTabName = item.FindElement(By.XPath("em/button/span[1]")).Text;
                        Console.WriteLine("Opened Analysis tab: " + analysisTabName);


                        switch (analysisTabName)
                        {
                            //Close "Overview Scenarios" tab
                            case "Overview Scenarios":
                                //executor.ExecuteScript("arguments[0].click();", driver.FindElement(By.XPath("//div[@id='TabPanel']/div[@class='x-tab-bar x-docked x-tab-bar-default x-top x-tab-bar-top x-tab-bar-default-top x-horizontal x-tab-bar-horizontal x-tab-bar-default-horizontal x-docked-top x-tab-bar-docked-top x-tab-bar-default-docked-top x-unselectable']/div[@class='x-tab-bar-body x-tab-bar-body-top x-tab-bar-body-default-top x-tab-bar-body-horizontal x-tab-bar-body-default-horizontal x-tab-bar-body-default x-tab-bar-body-default-top x-tab-bar-body-default-horizontal x-tab-bar-body-default-docked-top x-box-layout-ct']/div[@class='x-box-inner x-horizontal-box-overflow-body']/div/div[@class='x-tab x-box-item x-tab-default x-icon-text-left x-tab-icon-text-left x-tab-default-icon-text-left x-top x-tab-top x-tab-default-top x-closable x-tab-closable x-tab-default-closable x-closable-top x-tab-closable-top x-tab-default-closable-top x-active x-tab-active x-tab-default-active x-top-active x-tab-top-active x-tab-default-top-active']/a")));
                                executor.ExecuteScript("arguments[0].click();", item.FindElement(By.XPath("a")));
                                Thread.Sleep(2000);
                                break;

                            //Close "Default" scenario tab
                            case "Default scenario":
                                //executor.ExecuteScript("arguments[0].click();", driver.FindElement(By.XPath("//div[@id='TabPanel']/div[@class='x-tab-bar x-docked x-tab-bar-default x-top x-tab-bar-top x-tab-bar-default-top x-horizontal x-tab-bar-horizontal x-tab-bar-default-horizontal x-docked-top x-tab-bar-docked-top x-tab-bar-default-docked-top x-unselectable']/div[@class='x-tab-bar-body x-tab-bar-body-top x-tab-bar-body-default-top x-tab-bar-body-horizontal x-tab-bar-body-default-horizontal x-tab-bar-body-default x-tab-bar-body-default-top x-tab-bar-body-default-horizontal x-tab-bar-body-default-docked-top x-box-layout-ct']/div[@class='x-box-inner x-horizontal-box-overflow-body']/div/div[@class='x-tab x-box-item x-tab-default x-icon-text-left x-tab-icon-text-left x-tab-default-icon-text-left x-top x-tab-top x-tab-default-top x-closable x-tab-closable x-tab-default-closable x-closable-top x-tab-closable-top x-tab-default-closable-top x-active x-tab-active x-tab-default-active x-top-active x-tab-top-active x-tab-default-top-active']/a")));
                                executor.ExecuteScript("arguments[0].click();", item.FindElement(By.XPath("a")));
                                Thread.Sleep(2000);
                            ...
                            //Close "Overview Carriers" tab
                            case "Overview Carriers":
                                //executor.ExecuteScript("arguments[0].click();", driver.FindElement(By.XPath("//div[@id='TabPanel']/div[@class='x-tab-bar x-docked x-tab-bar-default x-top x-tab-bar-top x-tab-bar-default-top x-horizontal x-tab-bar-horizontal x-tab-bar-default-horizontal x-docked-top x-tab-bar-docked-top x-tab-bar-default-docked-top x-unselectable']/div[@class='x-tab-bar-body x-tab-bar-body-top x-tab-bar-body-default-top x-tab-bar-body-horizontal x-tab-bar-body-default-horizontal x-tab-bar-body-default x-tab-bar-body-default-top x-tab-bar-body-default-horizontal x-tab-bar-body-default-docked-top x-box-layout-ct']/div[@class='x-box-inner x-horizontal-box-overflow-body']/div/div[1]/a")));
                                executor.ExecuteScript("arguments[0].click();", item.FindElement(By.XPath("a")));
                                Thread.Sleep(2000);
                                break;
            ...
            {
                logger.Error("Problem encountered while checking currently opened tabs in the ANALYSIS: " + e.Message);
                driver.Close();
                driver.Quit();
            }
                ...
                driver.SwitchTo().Frame(driver.FindElement(By.XPath("//iframe[contains(@src,'/Analysis/ScenarioResultsTab?')]")));
                  ...
                  //Check that the upper grid has all expected carriers
                  try
                  {                    
                      IList<IWebElement> available_RowsInUpperGrid = driver.FindElements(By.XPath("//div[@id='ScenarioGridPanel-body']/div/table[@class='x-grid-table x-grid-table-resizer']/tbody/tr"));

                      //Loop the rows in the table of the upper grid
                      foreach (IWebElement item in available_RowsInUpperGrid)
                      {                      
                         RowCount++;
                      }
                  ...
                  {
                      logger.Error("Problem encountered while reviewing available rows in the upper grid of 'ManAlloc_SOC' scenario: " + e.Message);
                    driver.Close();
                    driver.Quit();
                }
                ...
                if (RowCount == numberOfCarriers)
                {
                    ...
                    if (numberOfCarriers == 4)  //Taking scrrenshots for Smoke test analysis
                    {
                        //Take screenshot of 'ManAlloc_SOC' scenario tab
                        TakeScreenshotOfWebElement("//div[@id='TabPanel']", "actual_shipper_ManAlloc_SOC_Tab.png");

                        //Compare master and actual screenshots and create a separate screenshot with differences
                        CompareScreenshots2("master_shipper_ManAlloc_SOC_Tab.png", "actual_shipper_ManAlloc_SOC_Tab.png", "difference_shipper_ManAlloc_SOC_Tab.png", 10);
                    }

                    else { //Taking screenshot for Next Tendering analysis
                        if (numberOfCarriers == 5)
                        {
                            //Take screenshot of 'ManAlloc_SOC' scenario tab
                            TakeScreenshotOfWebElement("//div[@id='TabPanel']", "actual_shipper_ManAlloc_SOC_Tab_NextTendering.png");

                            //Compare master and actual screenshots and create a separate screenshot with differences
                            CompareScreenshots2("master_shipper_ManAlloc_SOC_Tab_NextTendering.png", "actual_shipper_ManAlloc_SOC_Tab_NextTendering.png", "difference_shipper_ManAlloc_SOC_Tab_NextTendering.png", 10);
                        }
                    }

                    ///////////////////////////////////////////////////////////////////
                    // Select COLUMN BASED ALLOCATION mode and EDIT ALLOCATION option
                    ///////////////////////////////////////////////////////////////////
                    driver.SwitchTo().Frame(driver.FindElement(By.XPath("//iframe[contains(@src,'/Analysis/ScenarioResultsTab')]")));


                    //Get all buttons in the toolbar in the lower grid on the "ManAlloc_SOC" scenario
                    ...
                    //Loop the "ManAlloc_SOC" scenrio buttons for the lower grid
                    foreach (IWebElement item in available_buttons)
                    {
                        ...
                        if (!item.GetAttribute("class").Equals("x-component x-box-item x-toolbar-item x-component-default"))
                        {
                            if (!item.GetAttribute("class").Equals("x-toolbar-separator x-box-item x-toolbar-item x-toolbar-separator-horizontal"))
                            {
                                ...
                                //Retrieve names of available buttons in the toolbar
                                String buttonName = item.FindElement(By.XPath("em/button/span")).Text;
                                Console.WriteLine("Button in 'ManAlloc_SOC' SCENARIO toolbar (lower grid): " + buttonName);

                                switch (buttonName)
                                {
                                    case "Refresh":
                                        btnRefresh_ManAllocScenario = item;
                                        break;

                                    case "Analysis":
                                        btnAnalysis_ManAllocScenario = item;
                                        break;

                                    case "Visible fields":
                                        btnVisibleFields_ManAllocScenario = item;
                                        break;
                    ...
                    //Click on ANALYSIS button to open the dropdown menu
                    var watch1 = Stopwatch.StartNew();
                    executor.ExecuteScript("arguments[0].click();", btnAnalysis_ManAllocScenario);
                    //Wait till page is reloaded
                    watch1.Stop();
                    var elapsedMs1 = watch1.ElapsedMilliseconds;
                    var res1 = (double)elapsedMs1 / 1000;
                    if (res1 > 10)
                    {
                        logger.Info(string.Format("#Warning It took quite a long time: {0} sec for ANALYSIS dropdown menu to open in the lower grid of scenario in 'ManAlloc_SOC' SCENARIO tab!", res1.ToString()));
                    }

                    Thread.Sleep(1000);


                    //Click on WORKSHEET "Germany" item in the ANALYSIS dropdown menu on "ManAlloc_SOC" scenrio
                    executor.ExecuteScript("arguments[0].click();", driver.FindElement(By.XPath("//div[contains(text(), 'Germany')]")));

                    explicitWaitsUntilElementLocated(15, "//div[@id='visibleFields']", "'ManAlloc_SOC' SCENARIO tab -> Lower Grid -> Toolbar update after selecting WORKSHEET");
                    Thread.Sleep(1000);


                    //Click on the Refresh button on the "ManAlloc_SOC" scenrio toolbar in the lower grid i.e. WORKSHEET - TOTAL_SUM
                    executor.ExecuteScript("arguments[0].click();", btnRefresh_ManAllocScenario);

                    //Wait till the WORKSHEET - TOTAL SUM table is loaded in the lower grid on the "ManAlloc_SOC" scenrio
                    explicitWaitsUntilElementLocated(15, "//div[@id='ScenarioTablePanel-body']/div[@id='ScenarioTablePanel-innerCt']/div[@id='ScenarioTablePanel-targetEl']/div[2]/div[2]/div/table/tbody/tr[11]", "'ManAlloc_SOC' SCENARIO tab -> Lower Grid -> 'WORKSHEET' -> 'Total SUM'");
                    ...
                    if (numberOfCarriers == 4)  //Taking scrrenshots for Smoke test analysis
                    {
                        ...
                        //Take screenshot of "ManAlloc_SOC" scenrio (i.e. WORKSHEET -> TOTAL_SUM view in the lower grid)
                        TakeScreenshotOfWebElement("//iframe[contains(@src,'/TenderShipper/TenderShipperView')]", "actual_shipper_ManAlloc_SOC_Scenario_LowerGrid_Worksheet_TotalSum.png");
                        
                        //Compare master and actual screenshots and create a separate screenshot with differences
                        CompareScreenshots2("master_shipper_ManAlloc_SOC_Scenario_LowerGrid_Worksheet_TotalSum.png", "actual_shipper_ManAlloc_SOC_Scenario_LowerGrid_Worksheet_TotalSum.png", "difference_shipper_ManAlloc_SOC_Scenario_LowerGrid_Worksheet_TotalSum.png", 10);


                    }

                    else
                    { //Taking screenshot for Next Tendering analysis
                        if (numberOfCarriers == 5)
                        {
                            //Take screenshot of "ManAlloc_SOC" scenrio (i.e. WORKSHEET -> TOTAL_SUM view in the lower grid)
                            TakeScreenshotOfWebElement("//iframe[contains(@src,'/TenderShipper/TenderShipperView')]", "actual_shipper_ManAlloc_SOC_Scenario_LowerGrid_Worksheet_TotalSum_NextTendering.png");

                            //Compare master and actual screenshots and create a separate screenshot with differences
                            CompareScreenshots2("master_shipper_ManAlloc_SOC_Scenario_LowerGrid_Worksheet_TotalSum_NextTendering.png", "actual_shipper_ManAlloc_SOC_Scenario_LowerGrid_Worksheet_TotalSum_NextTendering.png", "difference_shipper_ManAlloc_SOC_Scenario_LowerGrid_Worksheet_TotalSum_NextTendering.png", 10);
                    ...
                    //Get all buttons in the toolbar in the lower grid on the "ManAlloc_SOC" scenario again to reach out to new buttons 'Column Based Allocation' and 'Edit allocation' 
                    IList<IWebElement> available_buttons2 = driver.FindElements(By.XPath("//div[@id='ScenarioTablePanel']/div[@class='x-toolbar x-docked x-toolbar-default x-docked-top x-toolbar-docked-top x-toolbar-default-docked-top x-box-layout-ct']/div[@class='x-box-inner ']/div/descendant::div"));


                    //Loop the "ManAlloc_SOC" scenrio buttons for the lower grid
                    foreach (IWebElement item2 in available_buttons2)
                    {

                        if (!item2.GetAttribute("class").Equals("x-component x-box-item x-toolbar-item x-component-default"))
                        {
                            if (!item2.GetAttribute("class").Equals("x-toolbar-separator x-box-item x-toolbar-item x-toolbar-separator-horizontal"))
                            {

                                //Retrieve names of available buttons in the toolbar
                                String buttonName2 = item2.FindElement(By.XPath("em/button/span")).Text;
                                Console.WriteLine("Button in 'ManAlloc_SOC' SCENARIO toolbar (lower grid): " + buttonName2);

                                switch (buttonName2)
                                {
                                    case "Refresh":
                                        btnRefresh_ManAllocScenario = item2;
                                        break;

                                    case "[Select allocation mode]":
                                        btnSelectAllocMode_ManAllocScenario = item2;
                                        break;

                                    case "[[View allocation]]":
                                        btnViewAllocMode_ManAllocScenario = item2;
                                        break;
                    ...
                    //Click on SELECT ALLOCATION MODE button to open the dropdown menu
                    var watch2 = Stopwatch.StartNew();
                    executor.ExecuteScript("arguments[0].click();", btnSelectAllocMode_ManAllocScenario);
                    //Wait till page is reloaded
                    watch2.Stop();
                    var elapsedMs2 = watch2.ElapsedMilliseconds;
                    var res2 = (double)elapsedMs2 / 1000;
                    if (res2 > 10)
                    {
                        logger.Info(string.Format("#Warning It took quite a long time: {0} sec for SELECT ALLOCATION MODE dropdown menu to open in the lower grid of scenario in 'ManAlloc_SOC' SCENARIO tab!", res2.ToString()));
                    }

                    Thread.Sleep(1000);


                    //Click on COLUMN BASED ALLOCATION item in the SELECT ALLOCATION MODE dropdown menu
                    executor.ExecuteScript("arguments[0].click();", driver.FindElement(By.XPath("//span[contains(text(), 'Column based allocation')]")));

                    Thread.Sleep(1000);



                    //Click on VIEW ALLOCATION MODE button to open the dropdown menu
                    var watch3 = Stopwatch.StartNew();
                    executor.ExecuteScript("arguments[0].click();", btnViewAllocMode_ManAllocScenario);
                    //Wait till page is reloaded
                    watch3.Stop();
                    var elapsedMs3 = watch3.ElapsedMilliseconds;
                    var res3 = (double)elapsedMs3 / 1000;
                    if (res3 > 10)
                    {
                        logger.Info(string.Format("#Warning It took quite a long time: {0} sec for VIEW ALLOCATION MODE dropdown menu to open in the lower grid of scenario in 'ManAlloc_SOC' SCENARIO tab!", res3.ToString()));
                    }
                    ...
                    //Click on COLUMN BASED ALLOCATION item in the SELECT ALLOCATION MODE dropdown menu
                    executor.ExecuteScript("arguments[0].click();", driver.FindElement(By.XPath("//span[contains(text(), 'Edit allocation')]")));

                    Thread.Sleep(1000);

                    //Wait till the WORKSHEET - TOTAL SUM table is loaded in the lower grid on the "ManAlloc_SOC" tab
                    explicitWaitsUntilElementLocated(15, "//div[@id='ScenarioTablePanel-body']/div[@id='ScenarioTablePanel-innerCt']/div[@id='ScenarioTablePanel-targetEl']/div[2]/div[2]/div/table/tbody/tr[11]", "'ManAlloc_SOC' SCENARIO tab -> Lower Grid -> 'WORKSHEET' -> 'Total SUM' after selecting allocation mode");
                    ...
                    if (numberOfCarriers == 4)  //Taking scrrenshots for Smoke test analysis
                    {
                        ...
                        TakeScreenshotOfWebElement("//iframe[contains(@src,'/TenderShipper/TenderShipperView')]", "actual_shipper_ManAlloc_SOC_Scenario_LowerGrid_AllocationMode.png");


                        //Compare master and actual screenshots and create a separate screenshot with differences
                        CompareScreenshots2("master_shipper_ManAlloc_SOC_Scenario_LowerGrid_AllocationMode.png", "actual_shipper_ManAlloc_SOC_Scenario_LowerGrid_AllocationMode.png", "difference_shipper_ManAlloc_SOC_Scenario_LowerGrid_AllocationMode.png", 10);
                    ...
                    { //Taking screenshot for Next Tendering analysis
                        ...
                        {
                            //Take screenshot of "Default Scenario" tab (i.e. WORKSHEET -> TOTAL_SUM view in the lower grid)
                            TakeScreenshotOfWebElement("//iframe[contains(@src,'/TenderShipper/TenderShipperView')]", "actual_shipper_ManAlloc_SOC_Scenario_LowerGrid_AllocationMode_NextTendering.png");


                            //Compare master and actual screenshots and create a separate screenshot with differences
                            CompareScreenshots2("master_shipper_ManAlloc_SOC_Scenario_LowerGrid_AllocationMode_NextTendering.png", "actual_shipper_ManAlloc_SOC_Scenario_LowerGrid_AllocationMode_NextTendering.png", "difference_shipper_ManAlloc_SOC_Scenario_LowerGrid_AllocationMode_NextTendering.png", 10);
                    ...
                    try
                    {
                        //Select the first row and count the columns containing 'Col' title
                        IList<IWebElement> available_numColumns = driver.FindElements(By.XPath("//div[@id='ScenarioTablePanel-targetEl']/div[2]/div[2]/div/table/thead/tr/th"));


                        //Loop the 1st row columns
                        foreach (IWebElement item in available_numColumns)
                        {
                            //Retrieve title of every column
                            String titleColumn = item.Text;
                            Console.WriteLine("Column title: " + titleColumn);

                            if (titleColumn.Contains("Col"))
                            {
                                numColumns++;
                    ...
                    {
                        logger.Error("Problem encountered while counting columns in the lower grid of 'ManAlloc_SOC' scenario: " + e.Message);

                    }


                    ////////////////////////////////////////////////////////////////////////////////////////////////////////
                    // Double-click on the FIRST column and start providing allocations into CHANGE COLUMN ALLOCATIONS form
                    ////////////////////////////////////////////////////////////////////////////////////////////////////////


                    //Click on the first column to select it
                    executor.ExecuteScript("arguments[0].click();", driver.FindElement(By.XPath("//div[@id='ScenarioTablePanel-body']/div[@id='ScenarioTablePanel-innerCt']/div[@id='ScenarioTablePanel-targetEl']/div[2]/div[2]/div/table/tbody/tr[1]/td[2]")));
                    Thread.Sleep(1000);

                    //Double-click on the first column to open CHANGE COLUMN ALLOCATIONS form
                    new Actions(driver).DoubleClick(driver.FindElement(By.XPath("//div[@id='ScenarioTablePanel-body']/div[@id='ScenarioTablePanel-innerCt']/div[@id='ScenarioTablePanel-targetEl']/div[2]/div[2]/div/table/tbody/tr[1]/td[2]"))).Perform();
                                        

                    if (numberOfCarriers == 4)  //Taking scrrenshots for Smoke test analysis
                    {
                        //Wait till the grid is loaded on the "CHANGE COLUMN ALLOCATIONS" form
                        waitTillDescendentElementsAvailable(15, "//table[@id='allocTable']/tbody/tr[12]/td[12]/div", "class", "red-highlighted-row", "'ManAlloc_SOC' SCENARIO tab -> 'CHANGE COLUMN ALLOCATION' form");
                        //explicitWaitsUntilElementLocated(15, "//div[@class='x-window x-layer x-window-default x-plain x-window-plain x-window-default-plain x-closable x-window-closable x-window-default-closable']/div[2]/div/table/tbody/tr[11]", "'ManAlloc_SOC' SCENARIO tab -> 'CHANGE COLUMN ALLOCATION' form");

                        Thread.Sleep(1000);

                        //Take screenshot of "CHANGE COLUMN ALLOCATIONS" form
                        TakeScreenshotOfWebElement("//div[@class='x-window x-layer x-window-default x-plain x-window-plain x-window-default-plain x-closable x-window-closable x-window-default-closable']", "actual_shipper_ManAlloc_SOC_Scenario_ChangeColumnAllocation_Form.png");
                        
                        //Compare master and actual screenshots and create a separate screenshot with differences
                        CompareScreenshots2("master_shipper_ManAlloc_SOC_Scenario_ChangeColumnAllocation_Form.png", "actual_shipper_ManAlloc_SOC_Scenario_ChangeColumnAllocation_Form.png", "difference_shipper_ManAlloc_SOC_Scenario_ChangeColumnAllocation_Form.png", 10);
                    ...
                    { //Taking screenshot for Next Tendering analysis
                        ...
                        {
                            //Wait till the grid is loaded on the "CHANGE COLUMN ALLOCATIONS" form
                            waitTillDescendentElementsAvailable(15, "//table[@id='allocTable']/tbody/tr[12]/td[16]/div", "class", "red-highlighted-row", "'ManAlloc_SOC' SCENARIO tab -> 'CHANGE COLUMN ALLOCATION' form");
                            //explicitWaitsUntilElementLocated(15, "//div[@class='x-window x-layer x-window-default x-plain x-window-plain x-window-default-plain x-closable x-window-closable x-window-default-closable']/div[2]/div/table/tbody/tr[11]", "'ManAlloc_SOC' SCENARIO tab -> 'CHANGE COLUMN ALLOCATION' form");

                            Thread.Sleep(1000);

                            //Take screenshot of "CHANGE COLUMN ALLOCATIONS" form
                            TakeScreenshotOfWebElement("//div[@class='x-window x-layer x-window-default x-plain x-window-plain x-window-default-plain x-closable x-window-closable x-window-default-closable']", "actual_shipper_ManAlloc_SOC_Scenario_ChangeColumnAllocation_Form_NextTendering.png");

                            //Compare master and actual screenshots and create a separate screenshot with differences
                            CompareScreenshots2("master_shipper_ManAlloc_SOC_Scenario_ChangeColumnAllocation_Form_NextTendering.png", "actual_shipper_ManAlloc_SOC_Scenario_ChangeColumnAllocation_Form_NextTendering.png", "difference_shipper_ManAlloc_SOC_Scenario_ChangeColumnAllocation_Form_NextTendering.png", 10);
                    ...
                    //Entering allocation % into the columns in the lower grid of 'ManAlloc_SOC' scenario
                    int columnCounter = 1;
                    int ColumnIndex = 0;
                    ...
                    try
                    {
                        ...
                        {
                            int twoTargetCarriers = 0;
                            int numberOfSubColumnsInEachColumn=0;

                            //Select the first row and find the column with the title of 'Thomas Roberts'
                            IList<IWebElement> available_numTitledColumns = driver.FindElements(By.XPath("//table[@id='allocTable']/thead/tr[1]/descendant::th"));

                            numberOfSubColumnsInEachColumn = available_numTitledColumns.Count;

                          
                                //Loop the 1st row columns
                                foreach (IWebElement item in available_numTitledColumns)
                                {
                                    //Retrieve title of every column
                                    String titleColumn2 = item.FindElement(By.XPath("div")).Text;
                                    Console.WriteLine("Column title on 'CHANGE COLUMN ALLOCATIONS' form : " + titleColumn2);

                                    ColumnIndex++;



                                    if (titleColumn2.Contains("Thomas Roberts") || titleColumn2.Contains("Lee Brant"))
                                    {
                                        if (titleColumn2.Contains("Thomas Roberts"))
                                        {
                                            //Give 70 % allocation to Thomas Roberts 
                                            executor.ExecuteScript("arguments[0].click();", driver.FindElement(By.XPath("//table[@id='allocTable']/thead/tr[3]/th[" + (ColumnIndex - 1) + "]/div/div/input")));
                                            driver.FindElement(By.XPath("//table[@id='allocTable']/thead/tr[3]/th[" + (ColumnIndex - 1) + "]/div/div/input")).Clear();
                                            driver.FindElement(By.XPath("//table[@id='allocTable']/thead/tr[3]/th[" + (ColumnIndex - 1) + "]/div/div/input")).SendKeys("70");

                                            twoTargetCarriers++;
                                        }


                                        if (titleColumn2.Contains("Lee Brant"))
                                        {
                                            //Give 30 % allocation to Robert Roberts (or Lee Brant if he is the only displayed carrier
                                            executor.ExecuteScript("arguments[0].click();", driver.FindElement(By.XPath("//table[@id='allocTable']/thead/tr[3]/th[" + (ColumnIndex - 1) + "]/div/div/input")));
                                            driver.FindElement(By.XPath("//table[@id='allocTable']/thead/tr[3]/th[" + (ColumnIndex - 1) + "]/div/div/input")).Clear();
                                            driver.FindElement(By.XPath("//table[@id='allocTable']/thead/tr[3]/th[" + (ColumnIndex - 1) + "]/div/div/input")).SendKeys("30");
                            ...
                            if (twoTargetCarriers == 2)
                            {

                                if (columnCounter < numColumns) //click on NEXT button if current column is not the last one
                                {
                                    //Click on the NEXT button on the "CHANGE COLUMN ALLOCATIONS" form to move to the next column for allocations
                                    executor.ExecuteScript("arguments[0].click();", driver.FindElement(By.XPath("//div[@class='x-btn x-box-item x-toolbar-item x-btn-default-small x-noicon x-btn-noicon x-btn-default-small-noicon']/em/button/span[contains(text(), 'Next')]")));

                                    //Wait till the grid is loaded on the "CHANGE COLUMN ALLOCATIONS" form
                                    // waitTillDescendentElementsAvailable(15, "//table[@id='allocTable']/tbody/tr[11]/td[17]/div", "class", "red-highlighted-row", "'ManAlloc_SOC' SCENARIO tab -> 'CHANGE COLUMN ALLOCATION' form -> Column# " + columnCounter);
                                    explicitWaitsUntilElementLocated(15, "//table[@id='allocTable']/tbody/tr[12]/td[4]/div[contains(@class,'orange-highlighted-row')]", "'ManAlloc_SOC' SCENARIO tab -> 'CHANGE COLUMN ALLOCATION' form -> Column# " + columnCounter);

                                    ColumnIndex = 0;
                                    Thread.Sleep(1000);
                                }
                                else
                                { //click on SAVE button if current column is the last one

                                    //Click on the SAVE button on the "CHANGE COLUMN ALLOCATIONS" form 
                                    executor.ExecuteScript("arguments[0].click();", driver.FindElement(By.XPath("//div[@class='x-btn x-box-item x-toolbar-item x-btn-default-small x-noicon x-btn-noicon x-btn-default-small-noicon']/em/button/span[contains(text(), 'Save')]")));

                                    //Wait till the WORKSHEET - TOTAL SUM table is loaded in the lower grid on the "ManAlloc_SOC" scenrio after saving manual allocations
                                    explicitWaitsUntilElementLocated(15, "//div[@id='ScenarioTablePanel-body']/div[@id='ScenarioTablePanel-innerCt']/div[@id='ScenarioTablePanel-targetEl']/div[2]/div[2]/div/table/tbody/tr[11]", "'ManAlloc_SOC' SCENARIO tab -> Lower Grid -> 'WORKSHEET' -> 'Total SUM' -> saved allocations");
                                    Thread.Sleep(1000);
                             ...
                             {
                                 if (numberOfSubColumnsInEachColumn == 2) //if current column does not have carriers with prices
                                 {                                    
                    ...
                    {
                        logger.Error("Problem encountered while entering allocation % into the columns in the lower grid of 'ManAlloc_SOC' scenario: " + e.Message);

                    }

                    ////////////////////////////////////////////////////////////////////////////////////////////////////////
                    //SAVE all provided allocations in the lower grid for 'ManAlloc_SOC' scenario
                    ////////////////////////////////////////////////////////////////////////////////////////////////////////

                    //Get all buttons in the toolbar in the lower grid on the "ManAlloc_SOC" scenario again to reach out to new buttons 'Column Based Allocation' and 'Edit allocation' 
                    IList<IWebElement> available_buttons3 = driver.FindElements(By.XPath("//div[@id='ScenarioTablePanel']/div[@class='x-toolbar x-docked x-toolbar-default x-docked-top x-toolbar-docked-top x-toolbar-default-docked-top x-box-layout-ct']/div[@class='x-box-inner ']/div/descendant::div"));


                    //Loop the "ManAlloc_SOC" scenrio buttons for the lower grid
                    foreach (IWebElement item3 in available_buttons3)
                    {

                        if (!item3.GetAttribute("class").Equals("x-component x-box-item x-toolbar-item x-component-default"))
                        {
                            if (!item3.GetAttribute("class").Equals("x-toolbar-separator x-box-item x-toolbar-item x-toolbar-separator-horizontal"))
                            {

                                //Retrieve names of available buttons in the toolbar
                                String buttonName3 = item3.FindElement(By.XPath("em/button/span")).Text;
                                Console.WriteLine("Button in 'ManAlloc_SOC' SCENARIO toolbar (lower grid): " + buttonName3);

                                switch (buttonName3)
                                {
                                    case "Save allocation":
                                        btnSaveAllocs_ManAllocScenario = item3;
                                        break;
                    ...
                    //Click on the 'Save allocations' button
                    executor.ExecuteScript("arguments[0].click();", btnSaveAllocs_ManAllocScenario);
                    Thread.Sleep(2000);

                    //Click on YES button to confirm saving
                    executor.ExecuteScript("arguments[0].click();", driver.FindElement(By.XPath("//div[@class='x-window x-message-box x-layer x-window-default x-closable x-window-closable x-window-default-closable']/div[3]/div/div/div/em/button/span[contains(text(), 'Yes')]")));


                    //Wait till the lower grid table with manual allocations is saved for "ManAlloc_SOC" scenario
                    explicitWaitsUntilElementLocated(15, "//div[@id='ScenarioTablePanel-body']/div[@id='ScenarioTablePanel-innerCt']/div[@id='ScenarioTablePanel-targetEl']/div[2]/div[2]/div/table/tbody/tr[11]", "'ManAlloc_SOC' SCENARIO tab -> Lower Grid -> ALL saved allocations");

                    //Wait till the upper grid table gets updated for "ManAlloc_SOC" scenario
                    explicitWaitsUntilElementLocated(15, "//div[@id='ScenarioGridPanel-body']/div/table[@class='x-grid-table x-grid-table-resizer']/tbody/tr[4]/td[9]", "'ManAlloc_SOC' SCENARIO tab -> Upper Grid -> Updated after saving all allocations");
                    Thread.Sleep(1000);
                    ...
                    if (numberOfCarriers == 4)  //Taking scrrenshots for Smoke test analysis
                    {
                        ...
                        //Take screenshot of "ManAlloc_SOC" tab 
                        TakeScreenshotOfWebElement("//div[@id='TabPanel']", "actual_shipper_ManAlloc_SOC_Scenario_SavedAllocations.png");
                        
                        //Compare master and actual screenshots and create a separate screenshot with differences
                        CompareScreenshots2("master_shipper_ManAlloc_SOC_Scenario_SavedAllocations.png", "actual_shipper_ManAlloc_SOC_Scenario_SavedAllocations.png", "difference_shipper_ManAlloc_SOC_Scenario_SavedAllocations.png", 10);
                    ...
                    { //Taking screenshot for Next Tendering analysis
                        ...
                        {
                            //Take screenshot of "ManAlloc_SOC" tab 
                            TakeScreenshotOfWebElement("//div[@id='TabPanel']", "actual_shipper_ManAlloc_SOC_Scenario_SavedAllocations_NextTendering.png");
                            
                            //Compare master and actual screenshots and create a separate screenshot with differences
                            CompareScreenshots2("master_shipper_ManAlloc_SOC_Scenario_SavedAllocations_NextTendering.png", "actual_shipper_ManAlloc_SOC_Scenario_SavedAllocations_NextTendering.png", "difference_shipper_ManAlloc_SOC_Scenario_SavedAllocations_NextTendering.png", 10);
                    ...
                    ////////////////////////////////////////////////////////////////////////////////////////////////////////
                    //Switch from COMPLETE OFFER to ALLOCATIONS view in the upper grid of 'ManAlloc_SOC' scenario
                    ////////////////////////////////////////////////////////////////////////////////////////////////////////
                    ...
                    //Get all buttons in the toolbar in the upper grid on the "ManAlloc_SOC" scenario
                    IList<IWebElement> available_buttonsUpperGrid = driver.FindElements(By.XPath("//div[@id='ScenarioGridPanel']/div[@class='x-toolbar x-docked x-toolbar-default x-docked-top x-toolbar-docked-top x-toolbar-default-docked-top x-box-layout-ct x-docked-noborder-top x-docked-noborder-right x-docked-noborder-left']/div/div/descendant::div"));


                    //Loop the "ManAlloc_SOC" scenrio buttons for the upper grid
                    foreach (IWebElement item4 in available_buttonsUpperGrid)
                    {

                        if (item4.GetAttribute("class").Equals("x-btn x-box-item x-toolbar-item x-btn-default-toolbar-small x-icon-text-left x-btn-icon-text-left x-btn-default-toolbar-small-icon-text-left"))
                        {                            

                          //Retrieve names of available buttons in the toolbar
                           String buttonName4 = item4.FindElement(By.XPath("em/button/span")).Text;
                           Console.WriteLine("Button in 'ManAlloc_SOC' SCENARIO toolbar (upper grid): " + buttonName4);

                        switch (buttonName4)
                        {
                            case "Complete Offer":
                                btnCompleteOffer_ManAllocScenario = item4;
                                break;
                    ...
                    //Click on COMPLETE OFFER button to open the dropdown menu
                    var watch4 = Stopwatch.StartNew();
                    executor.ExecuteScript("arguments[0].click();", btnCompleteOffer_ManAllocScenario);
                    //action.MoveToElement(btnCompleteOffer_ManAllocScenario).Click().Build().Perform();
                    ...
                    if (res4 > 10)
                    {
                        logger.Info(string.Format("#Warning It took quite a long time: {0} sec for COMPLETE OFFER dropdown menu to open in the upper grid of scenario in 'ManAlloc_SOC' SCENARIO tab!", res1.ToString()));
                    }

                    Thread.Sleep(1000);


                    //Click on "Allocations" item in the COMPLETE OFFER dropdown menu on "ManAlloc_SOC" scenrio
                    executor.ExecuteScript("arguments[0].click();", driver.FindElement(By.XPath("//div/a/span[contains(text(), 'Allocations')]")));

                    //Wait till the upper grid table with "Allocations" gets updated for "ManAlloc_SOC" scenario
                    explicitWaitsUntilElementLocated(15, "//div[@id='ScenarioGridPanel-body']/div/table[@class='x-grid-table x-grid-table-resizer']/tbody/tr[3]/td[9]", "'ManAlloc_SOC' SCENARIO tab -> Upper Grid -> Allocations view");
                    Thread.Sleep(1000);
                    ...
                    if (numberOfCarriers == 4)  //Taking scrrenshots for Smoke test analysis
                    {
                        ...
                        //Take screenshot of "ManAlloc_SOC" scenrio (i.e. Allocations view in the upper grid)
                        TakeScreenshotOfWebElement("//div[@id='ScenarioGridPanel']", "actual_shipper_ManAlloc_SOC_Scenario_UpperGrid_AllocationsView.png");
                        
                        //Compare master and actual screenshots and create a separate screenshot with differences
                        CompareScreenshots2("master_shipper_ManAlloc_SOC_Scenario_UpperGrid_AllocationsView.png", "actual_shipper_ManAlloc_SOC_Scenario_UpperGrid_AllocationsView.png", "difference_shipper_ManAlloc_SOC_Scenario_UpperGrid_AllocationsView.png", 10);
                    ...
                    { //Taking screenshot for Next Tendering analysis
                        ...
                        {
                            //Take screenshot of "ManAlloc_SOC" scenrio (i.e. Allocations view in the upper grid)
                            TakeScreenshotOfWebElement("//div[@id='ScenarioGridPanel']", "actual_shipper_ManAlloc_SOC_Scenario_UpperGrid_AllocationsView_NextTendering.png");

                            //Compare master and actual screenshots and create a separate screenshot with differences
                            CompareScreenshots2("master_shipper_ManAlloc_SOC_Scenario_UpperGrid_AllocationsView_NextTendering.png", "actual_shipper_ManAlloc_SOC_Scenario_UpperGrid_AllocationsView_NextTendering.png", "difference_shipper_ManAlloc_SOC_Scenario_UpperGrid_AllocationsView_NextTendering.png", 10);
                ...
                else
                {
                    ...
                    logger.Error("'ManAlloc_SOC' scenario is displaying incorrect number of carriers in the upper grid.");
                    driver.Close();
                    driver.Quit();
            ...
            else
            {
                ...
                logger.Error("'ManAlloc_SOC' scenario is not found in the DEFAULT folder of the ANALYSIS.");
                driver.Close();
                driver.Quit();
            }
        ...
        private void DeleteNumberOfCarriersRule()
        {
            Actions action = new Actions(driver);
            IJavaScriptExecutor executor = (IJavaScriptExecutor)driver;
            ...
            //Check if "Delete Rule" button got enabled in the toolbar and click on it (if it is enabled)
            String value = driver.FindElement(By.XPath("//div[@id='ScenarioTreePanel']/div/div/div/div[3]/em/button")).GetAttribute("disabled");
            if (value == null)
            {

                var buttonDeleteRule = driver.FindElement(By.XPath("//div[@id='ScenarioTreePanel']/div/div/div/div/em/button/span[contains(text(),'Delete rule')]"));
                executor.ExecuteScript("arguments[0].click();", buttonDeleteRule);

                Thread.Sleep(1000);

                //Click "Yes" on "Delete Rule" confirmation dialog
                var buttonYESDeleteRule = driver.FindElement(By.XPath("//div[@class='x-window x-message-box x-layer x-window-default x-closable x-window-closable x-window-default-closable']/div[@class='x-toolbar x-docked x-toolbar-footer x-docked-bottom x-toolbar-docked-bottom x-toolbar-footer-docked-bottom x-box-layout-ct']/div/div/div/em/button/span[contains(text(),'Yes')]"));
                executor.ExecuteScript("arguments[0].click();", buttonYESDeleteRule);
                             

                //Wait till the tender tree is loaded on the Step#2 of "Overview Scenarios" tab after deleting 'Number of Carriers' rule
                explicitWaitsUntilElementLocated(15, "//div[@id='ScenarioTreePanel-body']/div/table/tbody/tr[8]", "OVERVIEW SCENARIOS tab - STEP#2 after deleting 'Number of Carriers' rule");
            ...
            {
                logger.Error("DELETE RULE button is disabled after having selected existing NUMBER OF CARRIERS rule!");
                driver.Close();
                driver.Quit();
            }
        ...
        private void EditDeleteButtonsGetEnabled()
        {
            ...
            if (value == null)
            {
                ...
                //Take screenshot of "Edit Rule - NUMBER of CARRIERS" form
        ...
        private void CreateSelectionOfCarriersRule()
        {
            ...
            //Compare master and actual screenshots and create a separate screenshot with differences
            CompareScreenshots2("master_shipper_CreateNewScenarios_SelectionOfCarriersRule.png", "actual_shipper_CreateNewScenarios_SelectionOfCarriersRule.png", "difference_shipper_CreateNewScenarios_SelectionOfCarriersRule.png", 15);
        ...
        private void CreatePriceLimitsRule()
        {
            ...
            driver.FindElement(By.XPath("//input[@id='MaxPrice-inputEl']")).SendKeys("92");
            ...
            driver.FindElement(By.XPath("//input[@id='MinPrice-inputEl']")).SendKeys("72");
        ...
        private void TestOverviewScenarios()
        {
            ...
            //Compare master and actual screenshots and create a separate screenshot with differences
            CompareScreenshots2("master_shipper_OverviewScenarios_GraphicalPanel.png", "actual_shipper_OverviewScenarios_GraphicalPanel.png", "difference_shipper_OverviewScenarios_GraphicalPanel.png", 15);
                ...
                if (driver.FindElement(By.XPath("//div[@id='AwardingScenarioFormPanel-targetEl']/div[@class='x-container x-box-item x-container-default x-box-layout-ct']/div/div/div[2]/em/button")).Enabled)
                {
                    ...
                    if (driver.FindElement(By.XPath("//div[@id='AwardingScenarioFormPanel-targetEl']/div[@class='x-container x-box-item x-container-default x-box-layout-ct']/div/div/div[3]/em/button")).Enabled)
                    {
                        ...
                        if (driver.FindElement(By.XPath("//div[@id='AwardingScenarioFormPanel-targetEl']/table[@class='x-field x-form-item x-box-item x-field-default x-vbox-form-item']/tbody/tr/td[@class='x-form-item-body x-form-cb-wrap']/input[@class='x-form-field x-form-checkbox']")).Enabled)
                        {
                              ...
                              do
                              {
                                ...
                                IList<IWebElement> sentZip = driver.FindElements(By.XPath("//div[@id='AwardingAllocationsTreePanel-body']/div[@class='x-grid-view x-fit-item x-grid-view-default x-unselectable']/table/tbody/tr[5]/td[6]/div/descendant::*"));
                                   ...
                                   if (sentZip.Count == 0)
                                   {
                                    ...
                                    Thread.Sleep(10000);
                                    count++;
                                    logger.Warn("Unable to locate ZIP link with awarding results for the awarded carrier on STEP#1 of AWARDING SCENARIO Wizard after " + count + " attempt!");
                            ...
                            {
                                logger.Warn("Error while trying to open STEP#1 of AWARDING SCENARIO Wizard to verify the availability of ZIP link: " + e.Message);
        ...
        private void AwardingProcessForNextRoundTendering()
        {
            ...
            explicitWaitsUntilElementLocated(15, "//div[@id='AwardingAllocationsTreePanel-body']/div[@class='x-grid-view x-fit-item x-grid-view-default x-unselectable']/table/tbody/tr[7]/td[2]/div[contains(text(), 'CCX DEMO Carrier 03(Robert Roberts)')]", "AWARDING SCENARIO wizard -> STEP #1");
            Thread.Sleep(1000);

            //Take screenshot of "Awarding Scenario" wizard STEP#1
            TakeScreenshotOfWebElement("//div[@class='x-window x-layer x-window-default x-closable x-window-closable x-window-default-closable']", "actual_shipper_AwardingScenario_Step1_NextTendering.png");
            Thread.Sleep(1000);

            //Compare master and actual screenshots and create a separate screenshot with differences
            CompareScreenshots2("master_shipper_AwardingScenario_Step1_NextTendering.png", "actual_shipper_AwardingScenario_Step1_NextTendering.png", "difference_shipper_AwardingScenario_Step1_NextTendering.png", 10);
            Thread.Sleep(1000);
            ...
            explicitWaitsUntilElementLocated(15, "//div[@id='AwardingAllocationsTreePanel-body']/div[@class='x-grid-view x-fit-item x-grid-view-default x-unselectable']/table/tbody/tr/td/div[contains(text(), 'CCX DEMO Carrier 03(Robert Roberts)')]", "AWARDING SCENARIO wizard -> STEP #5_NextTendering");
            Thread.Sleep(1000);

            //Take screenshot of "Awarding Scenario" wizard STEP#5
            TakeScreenshotOfWebElement("//div[@class='x-window x-layer x-window-default x-closable x-window-closable x-window-default-closable']", "actual_shipper_AwardingScenario_Step5_NextTendering.png");
            Thread.Sleep(1000);

            //Compare master and actual screenshots and create a separate screenshot with differences
            CompareScreenshots2("master_shipper_AwardingScenario_Step5_NextTendering.png", "actual_shipper_AwardingScenario_Step5_NextTendering.png", "difference_shipper_AwardingScenario_Step5_NextTendering.png", 10);
            Thread.Sleep(1000);

            //Click on check-boxes to select Thomas Roberts and LeeBrant 
            executor.ExecuteScript("arguments[0].click();", driver.FindElement(By.XPath("//div[@id='AwardingAllocationsTreePanel-body']/div[@class='x-grid-view x-fit-item x-grid-view-default x-unselectable']/table/tbody/tr/td/div[contains(text(), 'Thomas Roberts')]")));
            //action.MoveToElement(driver.FindElement(By.XPath("//div[@id='AwardingAllocationsTreePanel-body']/div[@class='x-grid-view x-fit-item x-grid-view-default x-unselectable']/table/tbody/tr/td/div[contains(text(), 'CCX DEMO Carrier 05(Thomas Roberts)')]"))).Click().Build().Perform();
            Thread.Sleep(1000);
            action.MoveToElement(driver.FindElement(By.XPath("//div[@id='AwardingAllocationsTreePanel-body']/div[@class='x-grid-view x-fit-item x-grid-view-default x-unselectable']/table/tbody/tr/td/div[contains(text(), 'Lee Brant')]"))).Click().Build().Perform();
            Thread.Sleep(2000);

           

            //Check whether the "Next Round" button is enabled
            if (driver.FindElement(By.XPath("//div[@id='AwardingScenarioFormPanel-targetEl']/div[@class='x-container x-box-item x-container-default x-box-layout-ct']/div/div/div[2]/em/button")).Enabled)
            {
                Console.WriteLine("NEXT TENDER button is enabled on last step of Awarding wizard.");

                //Click on the "Next Tender" button
                executor.ExecuteScript("arguments[0].click();", driver.FindElement(By.XPath("//div[@id='AwardingScenarioFormPanel-targetEl']/div[@class='x-container x-box-item x-container-default x-box-layout-ct']/div/div/div[2]/em/button")));

                //Check that Thomas Roberts and LeeBrant are highlighted in orange color and marked as selected for Next Tendering round
               if (driver.FindElement(By.XPath("//div[@id='AwardingAllocationsTreePanel-body']/div[@class='x-grid-view x-fit-item x-grid-view-default x-unselectable']/table/tbody/tr[5]/td[2]")).GetAttribute("class").Contains("orange-highlighted-awarding-row") && driver.FindElement(By.XPath("//div[@id='AwardingAllocationsTreePanel-body']/div[@class='x-grid-view x-fit-item x-grid-view-default x-unselectable']/table/tbody/tr[6]/td[2]")).GetAttribute("class").Contains("orange-highlighted-awarding-row"))
                { 
                ...
                if (driver.FindElement(By.XPath("//div[@id='AwardingScenarioFormPanel-targetEl']/table[@class='x-field x-form-item x-box-item x-field-default x-vbox-form-item']/tbody/tr/td[@class='x-form-item-body x-form-cb-wrap']/input[@class='x-form-field x-form-checkbox']")).Enabled)
                {
                    Console.WriteLine("MOVE CARRIERS TO NEXT ROUND check-box is enabled on last step of Awarding wizard.");
                        ...
                        //Click to select the "Move carriers to next round including submitted prices" check-box
                        executor.ExecuteScript("arguments[0].click();", driver.FindElement(By.XPath("//div[@id='AwardingScenarioFormPanel-targetEl']/table[@class='x-field x-form-item x-box-item x-field-default x-vbox-form-item']/tbody/tr/td[@class='x-form-item-body x-form-cb-wrap']/input[@class='x-form-field x-form-checkbox']")));
                        Thread.Sleep(2000);
                            ...
                            else
                            {
                                foreach (IWebElement item in sentZip)
                                {
                                    if (item.Text.Equals("Zip"))
                                    {
                                        ...
                                        //Take screenshot of "Awarding Scenario" wizard STEP#1 with sent awarding results
                                        TakeScreenshotOfWebElement("//div[@class='x-window x-layer x-window-default x-closable x-window-closable x-window-default-closable']", "actual_shipper_AwardingScenario_Step1_WithSentResults_NextTendering.png");
                                        Thread.Sleep(1000);

                                        //Compare master and actual screenshots and create a separate screenshot with differences
                                        CompareScreenshots2("master_shipper_AwardingScenario_Step1_WithSentResults_NextTendering.png", "actual_shipper_AwardingScenario_Step1_WithSentResults_NextTendering.png", "difference_shipper_AwardingScenario_Step1_WithSentResults_NextTendering.png", 10);
                                        Thread.Sleep(1000);
                                        ...
                                        logger.Info("ZIP link was detected on Step#1 of AWARDING SCENARIO wizard. Awarding results have been successfully sent for carriers participating in the Next Tendering round.");
                    ...
                    {
                        logger.Warn("Error while trying to open STEP#1 of AWARDING SCENARIO Wizard to verify the availability of ZIP link for carriers participating in the Next Tendering round: " + e.Message);
                ...
                {
                    logger.Warn("MOVE CARRIERS TO THE NEXT ROUND check-box is DISABLED on STEP#5 of AWARDING SCENARIO Wizard for the Next Tendering round!");
                }

            }
            else
            {
                logger.Warn("Thomas Roberts and Lee Brant are not marked as selected for the Next Tendering round on STEP#5 of AWARDING SCENARIO Wizard!");
            }
            ...
            {
                logger.Warn("NEXT ROUND button is DISABLED on STEP#5 of AWARDING SCENARIO Wizard!");
            }
        ...
        private void FindAndApprovePublishedTender()
        {
            ...
            var btnApproveTender = driver.FindElement(By.XPath("//div[@class='x-window x-layer x-window-default x-plain x-window-plain x-window-default-plain x-closable x-window-closable x-window-default-closable']/div[@class='x-toolbar x-docked x-toolbar-footer x-docked-bottom x-toolbar-docked-bottom x-toolbar-footer-docked-bottom x-box-layout-ct']/div/div/div[1]/em/button/span[contains(text(), 'Accept')]"));
        ...
        private void FindAndAcceptPublishedNextRoundTender(string username, string matrixFilelocation)
        {
            // use js executor as click have problem in IE
            IJavaScriptExecutor executor = (IJavaScriptExecutor)driver;
            ...
            if (driver.FindElements(By.XPath("//div[@id='CarrierGridPanel-body']/div/table/tbody/descendant::tr")).Count() <= 1) // check if web-element is still displayed/open
            {
                logger.Warn("Carrier " + username + " is not allowed to particiapte in the NEXT ROUND TENDER.");
        ...
        private void SubmitPricesByCarrierIntoNextRoundTender(String username, String password, String matrixFilelocation)
        {
            LogInUserCarrier(username, password, "//iframe[contains(@src,'/TenderList/CarrierGridView')]");
            CloseUserProfileForm(username);
            SelectEnglishLanguage(10, "//iframe[contains(@src,'/TenderList/CarrierGridView')]");
            FindAndAcceptPublishedNextRoundTender(username, matrixFilelocation);          
        ...
        private void TestingAnalysisPartsAndAwarding()
        {
            ...
            MakingManualAllocationsInLowerGrid(4);

        }



        private void MakingAllocationsAndAwarding()
        {
            CheckAvailableAnalysisParts();
            //TestOverviewCarriers(); //not needed for next round tendering
            //TestOverviewScenarios(); //not needed for next round tendering
            //TestLowerGridOfDefaultScenario(); //not needed for next round tendering
            CreateManualAllocationScenarioWithoutFilters();
            MakingManualAllocationsInLowerGrid(5);
            AwardingProcessForNextRoundTendering();
        ...
        {
            /*
            // local path to CCX files  
            string projectCurrentPath1 = Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().CodeBase);
            var FilePath1 = Path.Combine(projectCurrentPath1, "binary_files\\c11.cmx");
            workPathCarrierMatrix1 = new Uri(FilePath1).LocalPath;
            Console.WriteLine("Actual Carrier's Matrix Path {0}", workPathCarrierMatrix1);

            var FilePath2 = Path.Combine(projectCurrentPath1, "binary_files\\c16.cmx");
            workPathCarrierMatrix2 = new Uri(FilePath2).LocalPath;

            var FilePath3 = Path.Combine(projectCurrentPath1, "binary_files\\c21.cmx");
            workPathCarrierMatrix3 = new Uri(FilePath3).LocalPath;*/
            ...
            // Just for test TO DO remove CloseCompatibilityModeWindow from here
               ...
               //SubmitPricesByCarrier("c11", "admin", workPathCarrierMatrix1);
               LogOut();
               WaitForLoad(driver);
               SubmitPricesByCarrier("c16", "admin", "D:/ccx3/Storage/c16.cmx");
               //SubmitPricesByCarrier("c16", "admin", workPathCarrierMatrix2);
               LogOut();
               WaitForLoad(driver);
               SubmitPricesByCarrier("c21", "admin", "D:/ccx3/Storage/c21.cmx");
               //SubmitPricesByCarrier("c21", "admin", workPathCarrierMatrix3);
               LogOut();
               WaitForLoad(driver);
        ...
        private void RunIENextRoundTenderingTest()
        {
            /*
            // local path to CCX files  
            string projectCurrentPath1 = Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().CodeBase);
            ...
            // Just for test TO DO remove CloseCompatibilityModeWindow from here
             ...
             WaitTillAnalysisGetsActivated();
             FindAndOpenTenderWithAnalysis();
             MakingAllocationsAndAwarding();
             FindAndOpenTenderWithAnalysisForNextTendering();
             CompletePreparationOfNextTenderingRound();
             FindCreatedNextRoundTender();
             EditAndPublishNextRoundTender();
             LogOut();
             WaitForLoad(driver);
             LogInUserAdmin("admin", "admin?987", "//iframe[contains(@src,'/TenderList/AdminGridView')]");
             FindAndApprovePublishedTender();
             LogOut();
             WaitForLoad(driver);
             SubmitPricesByCarrierIntoNextRoundTender("c16", "admin", "D:/ccx3/Storage/c16.cmx"); //participant1
             LogOut();
             WaitForLoad(driver);
             SubmitPricesByCarrierIntoNextRoundTender("c21", "admin", "D:/ccx3/Storage/c21.cmx"); //participant2
             LogOut();
             WaitForLoad(driver);
             SubmitPricesByCarrierIntoNextRoundTender("c34", "admin", "D:/ccx3/Storage/c34.cmx"); //participant3
             LogOut();
             WaitForLoad(driver);
             SubmitPricesByCarrierIntoNextRoundTender("c11", "admin", "D:/ccx3/Storage/c11.cmx"); //excluded from participation
             LogOut();
             WaitForLoad(driver);
             SubmitPricesByCarrierIntoNextRoundTender("c30", "admin", "D:/ccx3/Storage/c30.cmx"); //not a participant
             LogOut();
             WaitForLoad(driver);
        }
        ...
        public void CompareScreenshots2(String masterFileName, String actualFileName, String differenceFileName, int Tollerance) 
        {
            ...
            else
            {
                ...
                logger.Warn("The " + masterFileName + " screenshot SIZE is NOT the SAME as the SIZE of " + actualFileName + " screenshot.");
